<template>
    ...
    <div class="relative bg-login w-screen h-screen bg-cover bg-no-repeat">
        ...
        <div class="relative h-full w-full pt-10 bg-opacity-90 bg-black">
            
            <div class="relative w-4/5 md:w-3/5 lg:w-2/5 bg-white mx-auto shadow-2xl rounded-xl overflow-hidden">                       
                
                <div class="relative flex flex-col md:flex-row justify-between items-start min-h-112">

                    <!-- Login Form -->
                    <form @submit.prevent="loginUser({login: Input.login})" class="bg-white absolute w-full md:w-2/5 h-full py-6 px-8 transition-all duration-700" method="POST">
                    ...
                    <!-- Registration Form -->
                    <form @submit.prevent="registerUser({register: Input.register})" :class="mobileFormSelector" class="bg-white absolute md:right-0 h-full w-full md:w-2/5 py-6 px-8 transition-all duration-700" method="post" id="signup_candidate">
                    ...
                    <div :class="formSelector" class="absolute h-full w-auto md:w-3/5 transition-all duration-700">
                        ...
                        <div class="absolute top-0 right-0 w-auto md:w-full h-full bg-white bg-opacity-75">
                            ...
                            <div :class="mobileFormBGColor" class="absolute md:bg-white md:relative top-0 text-pink-800 right-0 h-full md:h-auto w-auto md:w-full flex flex-col md:flex-row justify-center md:justify-between px-2 md:px-6 md:py-5 text-2xl">
                            ...
                            <div class="hidden md:block relative mx-auto text-center text-gray-600">
                                <div :class="formTextColor" class="text-2xl uppercase font-bold p-4">Welcome to Career Strive</div>
                                <div class="text-2xl p-8">Submit your login credentials to gain access to future opportunities</div>
                                <button @click="toggleForm" :class="formBGColor" class="relative inline-block text-white shadow text-2xl px-8 py-3 rounded-xl">
                                    Register
                                </button>
                            </div>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            formSelector() {
                return this.toggler ? '' : 'right-0'
            },
            mobileFormSelector() {
                return this.mobileToggler ? '' : '-right-full'
            },
            formBGColor() {
                return this.toggler ? 'bg-green-400' : 'bg-blue-400'
            },
            mobileFormBGColor() {
                return this.mobileToggler ? 'bg-green-400' : 'bg-blue-400'
            },
            formTextColor() {
